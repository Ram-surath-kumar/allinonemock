...
export default function LibraryDashboard() {
  ...
  const handleIssue = async () => {
    if (!memberId || !copyId) {
      toast({
        ...
        description: "Member ID and Copy ID are required",
        variant: "destructive",
      });
      return;
    }
  ...
  const handleReturn = async () => {
    if (!copyId) {
      toast({
        title: "Validation Error",
        description: "Copy ID is required",
        variant: "destructive",
      });
      return;
    }
                ...
                <div className="space-y-2">
                  ...
                  {activeMember && (
                    <div className="text-sm p-2 bg-muted rounded-md flex justify-between">
                      <span>
                        Books: {activeMember.current_issued_count}/{activeMember.max_books_limit}
                      </span>
                      <span
                        className={
                          activeMember.status === "ACTIVE" ? "text-green-600" : "text-red-600"
                        }
                      >
                        {activeMember.status}
                      </span>
            ...
            <Card>
              ...
              <CardContent className="space-y-4">
                ...
                <div className="p-4 bg-yellow-50 rounded-md border border-yellow-200 text-yellow-800 text-sm flex gap-2">
                  ...
                  <p>
                    Late returns will automatically generate fines based on the overdue duration.
                  </p>
                </div>
