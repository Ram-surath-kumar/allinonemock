            ...
            <div className="flex flex-col md:flex-row gap-4 items-end">
                ...
                <div className="w-full md:w-1/3 space-y-2">
                    ...
                    <Select value={selectedCenter} onValueChange={setSelectedCenter}>
                        ...
                        <SelectContent>
                            {centers.map((center) => (
                                <SelectItem key={center.id} value={center.id}>{center.name} (Cap: {center.capacity})</SelectItem>
                            ))}
                        </SelectContent>
                    </Select>
                </div>

                <Button onClick={handleGenerate} disabled={loading || !selectedExam} className="shad-button-primary">
                    ...
                    {seatingPlan.length === 0 ? (
                        ...
                        <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-4">
                            {seatingPlan.map((seat, index) => (
                                <div key={index} className="border rounded-lg p-3 text-center bg-card hover:bg-accent/50 transition-colors">
                                    <div className="text-xs font-bold text-muted-foreground">{seat.room_number} - {seat.seat_number}</div>
