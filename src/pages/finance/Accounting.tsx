...
function ChartOfAccounts({ coa, onUpdate }: { coa: any[], onUpdate: () => void }) {
    ...
    // Group by Type
    const grouped = filteredCoa.reduce((acc, curr) => {
            ...
            <CardContent className="space-y-6">
                ...
                <div className="border rounded-lg overflow-hidden">
                    ...
                    <div className="bg-muted/30 p-2 flex justify-center border-t">
                        <Button variant="outline" size="sm" onClick={addLine} className="text-primary hover:text-primary">
                ...
                {difference !== 0 && (
                    <div className="flex justify-end text-destructive text-sm font-medium items-center">
                        <span className="bg-destructive/10 px-3 py-1 rounded-full">
                            Difference: {Math.abs(difference).toFixed(2)} {difference > 0 ? '(Dr > Cr)' : '(Cr > Dr)'}
                        </span>
                    </div>
                )}
            </CardContent>
            <CardFooter className="flex justify-end space-x-4 border-t pt-6">
                <Button variant="outline" onClick={() => setLines([{ account_id: '', debit: '', credit: '', description: '' }, { account_id: '', debit: '', credit: '', description: '' }])}>
                    Reset
                </Button>
                <Button onClick={handleSubmit} disabled={Math.abs(difference) > 0.01 || loading} className="min-w-[150px]">
                    {loading ? "Posting..." : <><Save className="mr-2 h-4 w-4" /> Post Journal</>}
                </Button>
            </CardFooter>
        </Card>
    );
