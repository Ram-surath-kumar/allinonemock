...
export default function HostelDashboard() {
    ...
    const handleDeleteHostel = async () => {
        ...
        try {
            setLoading(true);
            const response = await fetch(`${import.meta.env.VITE_API_URL || 'http://localhost:3001/api'}/hostel/${hostelToDelete.id}`, {
                method: 'DELETE'
            });
                                        ...
                                        <div key={allocation.id} className="flex items-center justify-between p-4 border rounded-lg hover:bg-muted/50 transition-colors">
                                            <div className="flex-1 space-y-1">
                                                <div className="flex items-center gap-2">
                                                    ...
                                                    <span className={`text-xs px-2 py-0.5 rounded-full ${allocation.status === 'active'
                                                ...
                                                <p className="text-sm text-muted-foreground flex items-center gap-2">
                                                    ...
                                                    {allocation.user?.loopid && (
                                                        <>
                                                            ...
                                                            <span className="font-mono text-xs bg-muted px-1.5 py-0.5 rounded">ID: {allocation.user.loopid}</span>
                                                        </>
                                                    )}
                                                </p>
                                                <div className="flex items-center gap-4 text-sm mt-2">
                                                    <div className="flex items-center gap-1">
                                                        ...
                                                        <span className="text-xs text-muted-foreground">({allocation.hostel?.type || 'N/A'})</span>
                                                    </div>
                                                    <div className="flex items-center gap-1">
                                                        <BedDouble className="h-4 w-4 text-muted-foreground" />
                                                        <span>Floor {allocation.room?.floor_number || '?'}, Room {allocation.room?.room_number || 'Unknown'}</span>
                                                    </div>
                                                </div>
                                                <p className="text-xs text-muted-foreground mt-1">
                                                    Allocated: {allocation.allocated_date ? new Date(allocation.allocated_date).toLocaleDateString('en-US', {
                                                        year: 'numeric',
                                                        month: 'long',
                                                        day: 'numeric'
                                                    }) : 'Date not available'}
                ...
                <TabsContent value="infrastructure">
                    <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
                        {data?.hostels?.map((hostel: any) => (
                            <Card key={hostel.id} className="hover:border-primary transition-colors relative group">
                                <CardHeader>
                ...
                <DialogContent>
                    <DialogHeader>
                        ...
                        <DialogDescription>
                            ...
                            This will also delete all associated rooms and allocations. This action cannot be undone.
                        </DialogDescription>
                    </DialogHeader>
                    <DialogFooter>
