...
export function UserManagement({ dialogOpen, setDialogOpen }) {
  ...
  const handleAddUser = async (newUser) => {
    try {
      ...
      const userData = {
        name: newUser.name,
        // Send college_email so backend can send the welcome email
      ...
      // Use API to create user (backend now handles ID generation, Auth user creation, and Email)
      ...
      if (data) {
        ...
        const user = {
          id: data.id,
          loopid: data.loopid, // Backend generated this
          org_id: data.org_id,
          user_id: data.user_id,
          name: data.name,
          email: data.email, // Backend generated this
          role: data.role,
        ...
        // Success message
        if (college_email) {
          toast.success(`User ${user.name} added and welcome email sent`);
        } else {
