...
export default function AdmissionAdmin() {
    const [stats, setStats] = useState({ total_applications: 0, pending_verification: 0, admitted: 0 });
    ...
    return (
        <div className="container mx-auto p-6 space-y-8">
            <div className="flex justify-between items-center">
                ...
                <Button onClick={fetchDashboardData} size="sm" variant="outline"><RefreshCcw className="h-4 w-4 mr-2" /> Refresh</Button>
            </div>
                            ...
                            {loading ? <div className="p-8 text-center"><Loader2 className="animate-spin h-8 w-8 mx-auto" /></div> : (
                                <div className="space-y-4">
                                    {applications.map(app => (
                                        <div key={app.id} className="flex justify-between items-center border-b pb-4 last:border-0">
                                            <div>
                                                <div className="font-semibold">{app.applicant_name}</div>
                                                <div className="text-sm text-muted-foreground">{app.application_no} | {app.course_applied}</div>
                                                <div className="flex gap-2 mt-1">
                                                    <Badge variant="outline">{app.gender || 'N/A'}</Badge>
                                                    <Badge variant="outline">{app.category || 'General'}</Badge>
                                                    <Badge className={
                                                        app.status === 'applied' ? 'bg-blue-100 text-blue-800' :
                                                            app.status === 'verified' ? 'bg-purple-100 text-purple-800' :
                                                                app.status === 'admitted' ? 'bg-green-100 text-green-800' : 'bg-gray-100 text-gray-800'
                                                    }>{app.status.toUpperCase()}</Badge>
                                                </div>
                                            </div>
                                            <div className="flex gap-2">
                                                {app.status === 'applied' && (
                                                    <Button size="sm" onClick={() => handleStatusUpdate(app.id, 'verified')}>Verify Docs</Button>
                                                )}
                                                {app.status === 'verified' && (
                                                    <Button size="sm" variant="secondary" onClick={() => handleStatusUpdate(app.id, 'shortlisted')}>Shortlist</Button>
                                                )}
                                                {app.status === 'merit_listed' && (
                                                    <Button size="sm" className="bg-green-600 hover:bg-green-700" onClick={() => handleStatusUpdate(app.id, 'admitted')}>Admit Student</Button>
                                                )}
                                            </div>
                                        </div>
                                    ))}
                            ...
                            <div className="grid gap-6 md:grid-cols-2">
                                <div className="space-y-4 border p-4 rounded-lg">
                                    ...
                                    <div className="space-y-2">
                                        ...
                                        <input className="w-full border rounded p-2 text-sm" placeholder="e.g. JEE Main 2026" id="examName" />
                                    </div>
                                    <div className="space-y-2">
                                        <label className="text-sm font-medium">Date</label>
                                        <input className="w-full border rounded p-2 text-sm" type="date" id="examDate" />
                                    </div>
                                ...
                                <div className="space-y-4 border p-4 rounded-lg">
                                    ...
                                    <div className="space-y-2">
                                        ...
                                        <input className="w-full border rounded p-2 text-sm" placeholder="UUID" id="scoreAppId" />
                                    </div>
                                    <div className="space-y-2">
                                        <label className="text-sm font-medium">Score</label>
                                        <input className="w-full border rounded p-2 text-sm" type="number" placeholder="e.g. 280" id="scoreValue" />
                                    </div>
                                    <Button onClick={async () => {
                                        ...
                                        toast.info("Select an exam from list to enter scores (Implementing logic...)");
                                    }}>Submit Score</Button>
                                </div>
                            </div>
                        </CardContent>
                            ...
                            <div className="grid gap-6 md:grid-cols-2">
                                <div className="space-y-4">
                                    <div className="space-y-2">
                                        ...
                                        <input className="w-full border rounded p-2 text-sm" defaultValue="Round 1 - CS" id="mlName" />
                                    </div>
                                    <div className="space-y-2">
                                    ...
                                    <div className="space-y-2">
                                        ...
                                        <input className="w-full border rounded p-2 text-sm" type="number" defaultValue="90" id="mlCutoff" />
                                    </div>
                                    <Button onClick={async () => {
                                ...
                                <div className="border rounded-lg p-4 bg-muted/20">
                                    ...
                                    <ul className="list-disc pl-4 text-sm text-muted-foreground space-y-1">
                                        ...
                                        <li>Students meeting the cutoff will be automatically moved to "Merit Listed" status.</li>
                                    </ul>
                                </div>
                            </div>
                        </CardContent>
