...
class ApiClient {
    ...
    async matchTransaction(data: { transaction_id: string; bank_transaction_id: string }): Promise<ApiResponse<any>> {
    ...
    async approveRefund(id: string, data: { status: 'approved' | 'rejected', approved_by: string }): Promise<ApiResponse<any>> {
    ...
    async createFeeHead(data: { name: string; type: string; is_refundable?: boolean }): Promise<ApiResponse<any>> {
    ...
    async assignFeeStructure(data: { student_id: string; structure_id: string; scholarship_id?: string }): Promise<ApiResponse<any>> {
    ...
    async getCollectionReport(params?: { start_date?: string; end_date?: string }): Promise<ApiResponse<any>> {
        const queryParams = new URLSearchParams();
