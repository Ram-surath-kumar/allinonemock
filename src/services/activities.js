...
const createNotificationForAdmins = async (title, message, type = "info") => {
  try {
    ...
    if (admins.length > 0) {
      ...
      await Promise.all(notifications.map((notification) => api.createNotification(notification)));
    }
  } catch (error) {
...
export const createUserAddedActivity = async (userName, department) => {
  ...
  try {
    await createNotificationForAdmins("New User Added", description, "info");
  } catch (error) {
...
export const createGradeUpdatedActivity = async (teacherName, subject) => {
  ...
  await createNotificationForAdmins("Grades Updated", description, "success");
};
...
export const createFeePaymentActivity = async (amount, payerName) => {
  ...
  // Create notification for admins
  await createNotificationForAdmins("Fee Payment Received", description, "success");
};
...
export const createEventScheduledActivity = async (eventName, date) => {
  ...
  // Create notification for admins
  await createNotificationForAdmins("Event Scheduled", description, "info");
};
