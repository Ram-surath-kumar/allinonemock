...
export function VehicleInventory() {
  ...
  const filteredVehicles = vehicles.filter((v) => {
    ...
    const matchesFilter =
      filterType === "all" || v.vehicle_type.toLowerCase() === filterType.toLowerCase();
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            Manage school fleet, maintenance and compliance.
          </p>
        </div>
              ...
              >
                <div className="p-5 space-y-4">
                  <div className="flex justify-between items-start">
                    ...
                    <div
                      className={`px-2 py-1 rounded text-[10px] font-bold uppercase tracking-wider ${getStatusColor(vehicle.status)}`}
                    >
                      {vehicle.status}
                    </div>
                  </div>
