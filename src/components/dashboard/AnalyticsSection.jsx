...
export function AnalyticsSection({ data }) {
  ...
  const renderChartView = (data, dataKey, xAxisKey, viewType, colors) => {
    if (viewType === "table") {
      return (
              ...
              <tr>
                ...
                {Object.keys(data[0] || {})
                  ...
                  .map((key) => (
                    <th
                      key={key}
                      className="text-right p-2 font-medium text-muted-foreground capitalize"
                    >
                      {key}
                    </th>
                  ))}
        ...
        <AreaChart {...commonProps}>
          <defs>
            <linearGradient id={`color${dataKey}`} x1="0" y1="0" x2="0" y2="1">
              <stop
                offset="5%"
                stopColor={colors?.primary || "hsl(var(--primary))"}
                stopOpacity={0.4}
              />
              <stop
                offset="95%"
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-3">
      {/* Attendance Chart */}
      <div
        className="rounded-2xl border border-border/30 bg-card/80 backdrop-blur-xl p-3.5 shadow-depth-2 hover:shadow-depth-3 transition-all duration-300 hover:scale-[1.01] hover:-translate-y-0.5 glass-modern"
        role="region"
      ...
      >
        ...
        {attendanceView === "table" ? (
          ...
          <ResponsiveContainer width="100%" height={200}>
            {renderChartView(attendanceData, "attendance", "day", attendanceView, {
      ...
      <div
        className="rounded-2xl border border-border/30 bg-card/80 backdrop-blur-xl p-3.5 shadow-depth-2 hover:shadow-depth-3 transition-all duration-300 hover:scale-[1.01] hover:-translate-y-0.5 glass-modern"
        role="region"
        aria-label="Fee Collection Chart"
      >
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <h3 className="text-sm font-semibold text-foreground">
                {t("dashboard.feeCollectionChart")}
              </h3>
        ...
        {feeView === "table" ? (
          ...
          <ResponsiveContainer width="100%" height={200}>
            {renderChartView(feeData, "collected", "month", feeView, {
      ...
      >
        ...
        {studentGrowthView === "table" ? (
          ...
          <ResponsiveContainer width="100%" height={200}>
            {renderChartView(studentGrowthData, "students", "month", studentGrowthView, {
              primary: "hsl(var(--primary))",
            })}
          </ResponsiveContainer>
        )}
      ...
      <div
        className="rounded-2xl border border-border/30 bg-card/80 backdrop-blur-xl p-3.5 shadow-depth-2 hover:shadow-depth-3 transition-all duration-300 hover:scale-[1.01] hover:-translate-y-0.5 glass-modern"
        role="region"
        aria-label="Today Summary Chart"
      >
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              ...
              <p className="text-xs text-muted-foreground">{t("dashboard.attendanceOverview")}</p>
            </div>
          </div>
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
