...
export function StudentAcademics({ userId }) {
  ...
  if (!userId)
    return (
      <div className="p-4 text-muted-foreground">Select a student to view academic details.</div>
    );
      ...
      <Tabs defaultValue="academic" className="w-full">
        <TabsList className="grid w-full grid-cols-3 h-auto">
          <TabsTrigger value="academic" className="py-3">
            <GraduationCap className="h-4 w-4 mr-2" /> Academic Record
          </TabsTrigger>
          <TabsTrigger value="leaves" className="py-3">
            <Plane className="h-4 w-4 mr-2" /> Leaves
          </TabsTrigger>
