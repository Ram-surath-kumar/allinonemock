  ...
  if (!audit || audit.message)
    return (
      <Alert>
        ...
        <AlertDescription>
          {audit?.message || "Could not retrieve graduation audit."}
        </AlertDescription>
      </Alert>
    );
      ...
      <div className="flex items-center justify-between">
        ...
        {status.is_eligible ? (
          <Badge className="bg-green-600 text-base py-1 px-3">
            <CheckCircle2 className="mr-2 h-4 w-4" /> Eligible for Graduation
          </Badge>
        ) : (
          <Badge
            variant="outline"
            className="text-base py-1 px-3 text-orange-600 border-orange-200 bg-orange-50"
          >
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <CardContent>
            <div className="flex justify-between items-end mb-2">
              ...
              <span className="text-sm text-muted-foreground mb-1">
                / {requirements.total_credits} Required
              </span>
            </div>
            <Progress value={status.credits_progress} className="h-3" />
            <p className="text-xs text-muted-foreground mt-2">
              {status.credits_earned >= requirements.total_credits
                ? "Requirement Met"
                : `${requirements.total_credits - status.credits_earned} credits remaining`}
            </p>
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent>
            <div className="flex justify-between items-end mb-2">
              <span
                className={`text-3xl font-bold ${status.current_cgpa < requirements.min_cgpa ? "text-destructive" : "text-green-600"}`}
              >
                {status.current_cgpa}
              </span>
              <span className="text-sm text-muted-foreground mb-1">
                / {requirements.min_cgpa} Min
              </span>
            </div>
            <Progress
              ...
              indicatorClassName={
                status.current_cgpa < requirements.min_cgpa ? "bg-destructive" : "bg-green-600"
              }
            />
            <p className="text-xs text-muted-foreground mt-2">
              {status.current_cgpa >= requirements.min_cgpa
                ? "Requirement Met"
                : "Below Minimum CGPA"}
            </p>
          </CardContent>
