...
export function CourseRegistration({ studentId, semesterId = "SEM-1" }) {
  ...
  const updateRegistration = (offeringId, field, value) => {
    setRegistrations((prev) =>
      prev.map((r) => (r.course_offering_id === offeringId ? { ...r, [field]: value } : r))
    );
  };
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        ...
        <Card className="min-w-[200px]">
          <CardContent className="p-4 flex items-center justify-between">
            ...
            <span
              className={`text-2xl font-bold ${totalCredits > 25 ? "text-destructive" : "text-primary"}`}
            >
              {totalCredits} <span className="text-sm text-muted-foreground">/ 25</span>
            </span>
          </CardContent>
        </Card>
      </div>
                  ...
                  {offerings.length === 0 ? (
                    ...
                    offerings.map((offering) => {
                      const isSelected = registrations.some(
                        (r) => r.course_offering_id === offering.id
                      );
                      const isAlreadyRegistered = myRegistrations.some(
                        (r) => r.course_offering_id === offering.id && r.status === "registered"
                      );
                        ...
                        <TableRow key={offering.id} className={isSelected ? "bg-muted/30" : ""}>
                                ...
                                <TooltipProvider>
                                  <Tooltip>
                                    <TooltipTrigger>
                                      ...
                                      >
                                        <ShieldAlert className="h-3 w-3" /> Prereq
                                      </Badge>
                                    </TooltipTrigger>
                                    <TooltipContent>
                                      <p className="text-xs">
                          ...
                          <TableCell>
                            <div className="text-sm space-y-1">
                              ...
                              <div
                                className={`text-xs ${isFull ? "text-destructive font-bold" : "text-green-600"}`}
                              >
                              ...
                              <div className="space-y-2 text-sm animate-in fade-in slide-in-from-top-1">
                                <div className="flex items-center gap-2">
                                  ...
                                  <select
                                    ...
                                    onChange={(e) =>
                                      updateRegistration(offering.id, "type", e.target.value)
                                    }
                                  >
                                  ...
                                  <div className="space-y-1">
                                    <div className="flex items-center gap-2">
                                      <Checkbox
                                        ...
                                        onCheckedChange={(c) =>
                                          updateRegistration(offering.id, "is_waiver_requested", c)
                                        }
                                      />
                                      <Label
                                        htmlFor={`waiver-${offering.id}`}
                ...
                <TableBody>
                  {myRegistrations.length === 0 ? (
                    ...
                    myRegistrations.map((reg) => (
                      <TableRow
                        key={reg.id}
                        className={reg.status === "dropped" ? "opacity-50 bg-muted/20" : ""}
                      >
                        <TableCell>
                        ...
                        <TableCell>
                          {reg.status === "registered" && (
                            <Button
                              variant="destructive"
                              size="sm"
                              onClick={() => handleDrop(reg.id, reg.courses?.course_code)}
                            >
                              Drop
                            </Button>
                          )}
                          {reg.status === "dropped" && (
                            <span className="text-xs text-muted-foreground">
                              Dropped on {new Date(reg.dropped_at).toLocaleDateString()}
                            </span>
                          )}
                        </TableCell>
                      </TableRow>
