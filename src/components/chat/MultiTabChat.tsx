...
import { MessageSquare, Phone, Video, X, Minimize2, Search, Check, CheckCheck } from "lucide-react";
        ...
        <ScrollArea className="flex-1">
          <div className="p-2 space-y-1">
            {recentChats.length === 0 ? (
              <p className="text-xs text-muted-foreground text-center py-4">No recent chats</p>
            ) : (
              recentChats.map((chat) => {
                ...
                return (
                  ...
                  >
                    {/* @ts-expect-error - Avatar components accept children */}
                    <Avatar className="h-8 w-8">
                      {/* @ts-expect-error - AvatarImage accepts src prop */}
                      <AvatarImage src={chat.userAvatar} />
                      {/* @ts-expect-error - AvatarFallback accepts children */}
