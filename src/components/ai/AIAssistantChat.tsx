...
export function AIAssistantChat({ onNavigate, isChatConversation, isChatPage }) {
    ...
    const executeAction = async (action, skipConfirmation = false) => {
        ...
        if (action.action === "mark_attendance") {
            ...
            try {
                ...
                let errorMessage = error instanceof Error ? error.message : "Failed to delete students";
            ...
            // Generic fallback: Try to find this action in the registry
            const availableActions = getActionsForUser(currentUser?.role);
            const registryAction = availableActions.find(
                a => a.id === action.action || a.title.toLowerCase() === action.action?.toLowerCase()
            );

            if (registryAction) {
                // Execute the registry action
                addMessage("assistant", `📍 ${registryAction.title}...`);

                if (onNavigate) {
                    onNavigate(registryAction.path || "/", registryAction.action, registryAction.actionData);
                } else if (registryAction.path) {
                    navigate(registryAction.path);
                }

                // Close chat after navigation
                if (registryAction.action === "navigate") {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            const isAnalyzeCommand = new RegExp("^(analyze system|analyze erp|analyze the entire erp)", "i").test(
                userPrompt
            );

            const isDataQuery =
                !isAnalyzeCommand &&
                (new RegExp("^(what|how|is|are|was|were|show|tell|give|list|display|explain|describe|compare|summary|report|count|check|find)", "i").test(
                    userPrompt
                ) ||
                    new RegExp("^(how many|how much|what is|what are|tell me|show me|give me|is there|are there)", "i").test(
                        userPrompt
                    ) ||
                    userPrompt.includes("?") ||
                    new RegExp("^(is|are|was|were)\\s+\\w+\\s+(present|absent|late|excused)", "i").test(userPrompt));
