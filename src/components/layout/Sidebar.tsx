...
export function Sidebar({ currentPath, onNavigate }: SidebarProps) {
  ...
  const fetchAllUsers = async () => {
    try {
      ...
      if (data) {
        const orgIds = [...new Set(data.filter((u: any) => u.org_id).map((u: any) => u.org_id))];
        const orgMap = new Map();
        
        if (orgIds.length > 0) {
          for (const orgId of orgIds) {
            const orgResponse = await api.getOrganizations({ id: String(orgId) });
            if (!orgResponse.error && orgResponse.data && orgResponse.data.length > 0) {
        ...
        const mappedUsers: User[] = data.map((row: any) => ({
          ...
          avatar: row.avatar,
          organization: row.org_id && orgMap.has(row.org_id) ? {
            id: orgMap.get(row.org_id).id,
            org_id: orgMap.get(row.org_id).org_id,
            org_code: orgMap.get(row.org_id).org_code,
            org_name: orgMap.get(row.org_id).org_name,
          } : undefined,
        }));
  ...
  return (
    ...
    >
      ...
      <div className={cn(
        ...
        <div className={cn(
          "flex items-center justify-center rounded-xl bg-gradient-to-br from-sidebar-primary to-sidebar-primary/80 shadow-md shrink-0 transition-transform duration-300 hover:scale-105 hover:shadow-glow",
        ...
        <div className={cn(
          ...
          <div className="whitespace-nowrap pl-3">
            <h1 className="text-sm font-bold tracking-tight text-sidebar-foreground">EduAdmin</h1>
